...
SQLitePlugin = function(openargs, openSuccess, openError) {
  openargs.dbConId = openargs.dbConId || openargs.name
  var dbname;
  ...
  this.dbname = dbname;
  this.dbConId = openargs.dbConId;
  this.openSuccess = openSuccess;
  this.openError = openError;
  this.openSuccess || (this.openSuccess = function() {
    plugin.log("DB opened: " + this.dbConId);
  });
...
SQLitePlugin.prototype.addTransaction = function(t) {
  if (!txLocks[this.dbConId]) {
    txLocks[this.dbConId] = {
      queue: [],
      inProgress: false
    };
  }
  txLocks[this.dbConId].queue.push(t);
  if (this.dbConId in this.openDBs && this.openDBs[this.dbConId] !== DB_STATE_INIT) {
    this.startNextTransaction();
  } else {
    if (this.dbConId in this.openDBs) {
...
SQLitePlugin.prototype.startNextTransaction = function() {
  ...
  nextTick((function(_this) {
    return function() {
      var txLock;
      if (!(_this.dbConId in _this.openDBs) || _this.openDBs[_this.dbConId] !== DB_STATE_OPEN) {
        plugin.log('cannot start next transaction: database not open');
        return;
      }
      txLock = txLocks[self.dbConId];
      if (!txLock) {
...
SQLitePlugin.prototype.open = function(success, error) {
  ...
  if (this.dbConId in this.openDBs && this.openDBs[this.dbConId] === DB_STATE_OPEN) {
    plugin.log('database already open: ' + this.dbConId);
    nextTick((function(_this) {
      return function() {
        success(_this);
      };
    ...
    plugin.log('OPEN database: ' + this.dbConId);
    opensuccesscb = (function(_this) {
      return function() {
        var txLock;
        if (!_this.openDBs[_this.dbConId]) {
          plugin.log('database was closed during open operation');
        }
        if (_this.dbConId in _this.openDBs) {
          _this.openDBs[_this.dbConId] = DB_STATE_OPEN;
        }
        if (!!success) {
          success(_this);
        }
        txLock = txLocks[_this.dbConId];
    ...
    openerrorcb = (function(_this) {
      return function() {
        plugin.log('OPEN database: ' + _this.dbConId + ' failed, aborting any pending transactions');
        if (!!error) {
          error(newSQLError('Could not open database'));
        }
        delete _this.openDBs[_this.dbConId];
        _this.abortAllPendingTransactions();
      };
    })(this);
    this.openDBs[this.dbConId] = DB_STATE_INIT;
...
SQLitePlugin.prototype.close = function(success, error) {
  if (this.dbConId in this.openDBs) {
    if (txLocks[this.dbConId] && txLocks[this.dbConId].inProgress) {
      plugin.log('cannot close: transaction is in progress');
      error(newSQLError('database cannot be closed while a transaction is in progress'));
      return;
    }
    plugin.log('CLOSE database: ' + this.dbConId);
    delete this.openDBs[this.dbConId];
    if (txLocks[this.dbConId]) {
      plugin.log('closing db with transaction queue length: ' + txLocks[this.dbConId].queue.length);
    } else {
    ...
    plugin.exec("close",{path: this.dbname, dbConId: this.dbConId }, mysuccess, myerror);
  } else {
...
SQLitePluginTransaction.prototype.start = function() {
  ...
  try {
    ...
    err = _error;
    txLocks[this.db.dbConId].inProgress = false;
...
SQLitePluginTransaction.prototype.run = function() {
  ...
  plugin.exec("backgroundExecuteSqlBatch",{
    name: this.db.dbname, // android
    dbConId: this.db.dbConId, // android
    dbargs: { // iOS
      dbname: this.db.dbname,
      dbConId: this.db.dbConId,
    },
    executes: tropts
  }, mysuccess, myerror);
};
...
SQLitePluginTransaction.prototype.abort = function(txFailure) {
  ...
  succeeded = function(tx) {
    txLocks[tx.db.dbConId].inProgress = false;
